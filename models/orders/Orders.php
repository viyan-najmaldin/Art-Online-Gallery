...
class Orders
{
     ...
     * @return bool
     */
    public function addOrderDetails(art $b, int $quantity):bool {
        if ( $b->buy($quantity)) {
    ...
    public function getTotalPrice():float {
        ...
        foreach ($this->orderDetails as  $detail) {
            $total += ($detail->getart()->getSellprice() * $detail->getQuantity());
        }
        return $total;
    }
    ...
    public function deleteOrderDetails (int $id)
    {
        if (!$this->is_deliver) {
              ...
              $detail->getart()->backart($detail->getQuantity());
       ...
       try{
           if ($this->is_deliver)
               ...
               foreach ($this->orderDetails as $key=>$val) {
                   $val->getart()->delivering_done($val->getQuantity());
               }
           }
    ...
    public function addUnAvailableOrderDetails(art $b,int $quantity){
