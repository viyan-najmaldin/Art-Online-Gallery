...
class Talking
{
    ...
    private static $pass = '';
    private static $dbname = 'art_store';
    private static $host = 'localhost';
    public const   ORDER_BY_NAME_A_Z = 1;
    public const   ORDER_BY_NAME_Z_A = 2;
    public const   ORDER_BY_PRICE = 3;
    public const   ORDER_BY_SELLING = 4;//common arts (best sellers)
    public const   SEARCH_art_NAME = 5;
    public const   SEARCH_AUTHOR_NAME = 6;
    public const   SEARCH_art_CATEGORY = 7;
     ...
     * @return art
     */
    public function getAvailablearts ()
    {
        try {
            $this->stat = $this->conn->prepare("
             SELECT a.name as artist, b.art_id,b.title,b.sell_price,b.ISBN,b.buy_price,p.name as publisher_name,p.publisher_id,b.quantity,b.actual_quantity,b.description,b.image
             FROM artist a JOIN art b ON a.artist_id=b.artist_id 
    ...
    public function bestSeller ($limt = 50): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT b.art_id,b.title,b.buy_price,b.image, a.name AS artist
            FROM art b JOIN order_details od on b.art_id = od.art_id
            JOIN artist a on b.artist_id = a.artist_id,orders o
            where o.order_date<TIMESTAMPADD(YEAR ,1,o.order_date)
            AND b.quantity>0
            GROUP by od.art_id
            ORDER BY COUNT(od.art_id) DESC LIMIT ?;");
...
FROM genre;");
    ...
    {
        try {
            $sql = "SELECT b.art_id,b.title,b.buy_price,b.image, a.name AS artist
              FROM art b JOIN artist a on b.artist_id = a.artist_id
              WHERE b.quantity>0 ";
            switch ($orderBy) {
                case 1:
                    $sql .= ' ORDER BY 2 ;';
                ...
                case 4:
                    $sql = "
SELECT b.art_id,b.title,b.buy_price,b.image, a.name AS artist,COUNT(b.art_id)
FROM art b JOIN order_details od on b.art_id = od.art_id
JOIN artist a on b.artist_id = a.artist_id,orders o
WHERE b.quantity>0
AND o.order_date<TIMESTAMPADD(YEAR ,1,o.order_date)
GROUP BY b.art_id,b.title
ORDER BY COUNT(b.art_id) DESC,2;";
                    break;
                default:
                    $sql .= ' ORDER BY 3,2;';
            }
...
WHERE a.name like ? AND  b.quantity>0;");
    ...
    public function getartInfo ($id): art
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.art_id,b.title,b.ISBN,b.sell_price,b.buy_price,b.image,b.quantity,b.actual_quantity,b.description,a.name as artist,p.name as publisher_name
FROM art b,artist a,publisher p
WHERE b.art_id=?
...
");
    ...
    public function getNotAvailablearts ()
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.art_id,b.title,b.sell_price,b.buy_price,b.description,b.actual_quantity,p.phone as publisher,p.phone as publisher_phone,sum(od.quantity) as co
from art b,order_details od,orders o,publisher p
where o.order_date <= TIMESTAMPADD(YEAR ,1,o.order_date)
AND b.art_id=od.art_id
AND b.quantity=0
AND p.publisher_id=b.publisher_id
...
ORDER BY sum(od.quantity) DESC ;");
    ...
    public function getartsOffPublisher (int $pubid): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT b.art_id,b.title,b.image,b.sell_price,b.buy_price,b.ISBN
            FROM publisher p JOIN art b on p.publisher_id = b.publisher_id
...
");
     ...
     * @return bool|PDOStatement: false if there no data in art table in DB,
    ...
    public function lastUpload (int $limit = 20)
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT b.art_id,b.title,b.sell_price,b.image,a.name AS artist
            FROM art b JOIN artist a on b.artist_id = a.artist_id
            WHERE b.quantity>0
    ...
    public function getOrderofCustomer ($customerId): Orders
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT art.art_id, art.title,art.ISBN,art.sell_price,art.buy_price,art.image,art.actual_quantity,art.quantity,a.name as artist,od.quantity as orderQuantity
            FROM customer join orders o on customer.customer_id = o.customer_id
                join order_details od on o.order_id = od.order_id
                JOIN art on od.art_id = art.art_id
                JOIN artist a on art.artist_id = a.artist_id
            ...
            while ($row = $this->stat->fetch(PDO::FETCH_ASSOC)) {
                $b = new art(
                    $row['art_id'], $row['title'],
                    $row['ISBN'], $row['sell_price'],
                    $row['buy_price'], $row['image'],
                    '', $row['actual_quantity'],
                    $row['quantity'], new artist($row['artist']));
     ...
     * @param string $str
     * if $search = $this->SEARCH_AUTHOR_NAME (6)it will search by artist by "like" method .
     * if $search = $this->SEARCH_art_CATEGORY(7) it will search by genre (it should be  the same as the name in database to get that data )
     * in other words it uses "=" unlike the above($this->SEARCH_AUTHOR_NAME,$this->SEARCH_art_CATEGORY)
     * and others .if $search $this->SEARCH_art_NAME (5)it will search by name(title) of the art  by "like" method.
     * if its not 6 and 7 it consider that it's 5
     * @return void and it cash the data in the shape of art objects.
     * you can only get result data from art::getObjects():array
     */

    public function casharts (int $search, string $str): void
    {
        if ($search == 7) {
            $query = "SELECT b.*,a.name AS artist
                    FROM genre JOIN art b 
                    ON genre.genre_id = b.genre_id JOIN artist a on b.artist_id = a.artist_id
                    WHERE genre_desc=?;";
            ...
            $query = "SELECT b.*,a.name AS artist
                      FROM art b JOIN artist a on b.artist_id = a.artist_id WHERE $type like ?;";
        }
        try {
...
/// they do not test
    ...
    public function getAuthor ($name): artist
    {
        try {
            $this->stat = $this->conn->prepare("
            ...
            $this->stat->execute();
            $row=$this->stat->fetch();
            return new artist($name,$row['artist_id']);
        } catch (Exception $PDOException) {
            echo $PDOException->getMessage();
        }
    }
