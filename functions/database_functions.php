<?php
	...
	function select4Latestart($conn){
		$row = array();
		$query = "SELECT art_isbn, art_image FROM arts ORDER BY art_isbn DESC";
	...
	function getartByIsbn($conn, $isbn){
		$query = "SELECT art_title, art_artist, art_price FROM arts WHERE art_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getartprice($isbn){
		$conn = db_connect();
		$query = "SELECT art_price FROM arts WHERE art_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "get art price failed! " . mysqli_error($conn);
			exit;
		}
		$row = mysqli_fetch_assoc($result);
		return $row['art_price'];
	}
	...
	function getPubName($conn, $pubid){
		...
		if(mysqli_num_rows($result) == 0){
			echo "Empty arts ! Something wrong! check again";
			exit;
		}
	...
	function getAll($conn){
		$query = "SELECT * from arts ORDER BY art_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
